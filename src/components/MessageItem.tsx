...
import IconCorrection from "./icons/Correction";
...
export default ({ role, message, showRetry, onRetry }: Props) => {
  ...
  return (
    <div class="py-2 -mx-4 px-4 transition-colors md:hover:bg-slate/3 word">
      <div class="flex gap-3 rounded-lg" class:op-75={role === "user"}>
        ...
        />
        {roleClass[role] === 'user' && (
          <button title="Correct Grammar" class="correction" gen-slate-btn-sm>
            <IconCorrection />
          </button>
        )}  
        {/* {roleClass[role] === 'assistant' && speak(htmlString())} */}
      </div>
      {showRetry?.() && onRetry && (
